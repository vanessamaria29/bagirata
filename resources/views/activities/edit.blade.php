...
<div class="max-w-2xl mx-auto py-10">
    ...
    <div class="bg-white p-10 rounded-[3rem] shadow-2xl border border-gray-50 relative overflow-hidden">
        ...
        <form action="{{ route('activities.update', $activity->id) }}" method="POST" class="space-y-8 relative z-10">
            ...
            <div>
                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 italic">Masukkan ke Trip (Opsional)</label>
                <select name="trip_id" class="w-full px-8 py-5 bg-gray-50 border-2 border-transparent focus:border-blue-100 focus:bg-white focus:ring-4 focus:ring-blue-50 rounded-[1.5rem] transition-all font-black text-gray-900 outline-none cursor-pointer">
                    <option value="">-- Tanpa Trip --</option>
                    @foreach($trips as $t)
                        <option value="{{ $t->id }}" {{ old('trip_id', $activity->trip_id) == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 italic">Pajak (PPN)</label>
                    <div class="relative">
                        <span class="absolute left-6 top-1/2 -translate-y-1/2 text-xs font-black text-gray-400">Rp</span>
                        <input type="number" name="tax" value="{{ old('tax', $activity->tax) }}" min="0" placeholder="0"
                            class="w-full pl-12 pr-8 py-5 bg-gray-50 border-2 border-transparent focus:border-blue-100 focus:bg-white focus:ring-4 focus:ring-blue-50 rounded-[1.5rem] transition-all font-black text-gray-900 outline-none">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 italic">Service Charge</label>
                    <div class="relative">
                        <span class="absolute left-6 top-1/2 -translate-y-1/2 text-xs font-black text-gray-400">Rp</span>
                        <input type="number" name="service_charge" value="{{ old('service_charge', $activity->service_charge) }}" min="0" placeholder="0"
                            class="w-full pl-12 pr-8 py-5 bg-gray-50 border-2 border-transparent focus:border-blue-100 focus:bg-white focus:ring-4 focus:ring-blue-50 rounded-[1.5rem] transition-all font-black text-gray-900 outline-none">
                    </div>
                </div>
            </div>
